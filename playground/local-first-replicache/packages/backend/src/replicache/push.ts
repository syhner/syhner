...
export const push = new Hono().post("/", async (c) => {
...
async function processMutation(
  ...
  if (error === undefined) {
    ...
    switch (mutation.name) {
      case "createMessage":
        await createMessage(t, mutation.args as Message, nextVersion);
        break;
      default:
  ...
  await t.none("update replicache_server set version = $1 where id = $2", [nextVersion, serverID]);
}
